...
import csv
from app.db.models import Batch, BatchPatient, Patient, Person, Visit
...
async def process_csv(file_name: str):
    ...
    async with AsyncSessionLocal() as session:
        async with session.begin():
            batch = Batch(filename=file_name)
            session.add(batch)
            await session.flush()
            ...
            with open(local_path) as f:
                ...
                for row in reader:
                    # async with session.begin():
                        ...
                        batch_patient = BatchPatient(batch_id=batch.id, patient_id=patient.id)
                        session.add(batch_patient)
